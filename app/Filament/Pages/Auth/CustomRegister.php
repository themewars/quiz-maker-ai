...
class CustomRegister extends Register
{
    ...
    public function register(): ?RegistrationResponse
    {
        ...
        if (env('AUTO_VERIFY_USERS', false)) {
            ...
            if ($msg === 'messages.user.register_successfully') {
            ...
            Notification::make()
                ->success()
                ->title((function(){
                    $t = __('messages.home.email_verification_link_sent');
                    return $t === 'messages.home.email_verification_link_sent' ? 'Verification email sent' : $t;
                })())
                ->send();
        }

        // Don't login user if email is not verified - let Filament handle the response
        if (!$user->hasVerifiedEmail()) {
            // Login user temporarily so RegistrationResponse can access auth()->user()
            auth()->login($user);
            // Let Filament registration response handle the redirect
            return null;
        }
