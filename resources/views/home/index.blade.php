                    ...
                    <div class="hero-text animate-fade-in">
                        ...
                        <h1 class="hero-title">
                            @if (!empty(getSetting()->hero_title))
                                <span
                                    class="gradient-text">{{ getSetting()->hero_title ?? __('messages.home.instantly_turn_documents_web') . ' ' . __('messages.home.content_into_interactive_exams') }}</span>
                            @else
        ...
        @if ($quizzes->count() > 0)
            ...
            <section id="examples" class="examples">
                <div class="container">
                    ...
                    <!-- Quiz content containers (original IDs/classes, UI text stays Exam) -->
                    <div id="all-quizzes" class="quiz-category-content active">
                        ...
                        <!-- Additional quizzes in grid (original structure) -->
                        <div class="quizzes-grid">
                            ...
                            @foreach ($allQuizzes as $quiz)
                                <div class="quiz-card animate-fade-in" data-delay="0.{{ $loop->index }}">
                                    <div class="quiz-card-header {{ $loop->index == 1 ? 'green-gradient' : ($loop->index == 2 ? 'orange-gradient' : 'indigo-gradient') }}"></div>
                    ...
                    @foreach ($categories as $id => $category)
                        <div id="{{ $id }}-quizzes" class="quiz-category-content">
                            ...
                            @endphp
                            <!-- All quizzes in this category in grid format (original structure) -->
                            <div class="quizzes-grid">
                                @foreach ($categoryQuizzes as $quiz)
                                    <div class="quiz-card animate-fade-in" data-delay="0.{{ $loop->index }}">
                                        <div class="quiz-card-header {{ $loop->index % 3 == 1 ? 'green-gradient' : ($loop->index % 3 == 2 ? 'orange-gradient' : 'indigo-gradient') }}"></div>
                                        <div class="quiz-card-content">
                                            <div class="quiz-card-top">
                                                <div
                        ...
                        @foreach ($plans as $plan)
                            <div class="pricing-card animate-fade-in {{ $loop->index == 1 ? 'popular' : '' }}">
                                ...
                                <ul class="pricing-features">
                                    ...
                                    <li class="feature-item">
                                        <svg class="feature-icon" width="20" height="20" viewBox="0 0 24 24" fill="none"><path d="M5 13l4 4L19 7" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" /></svg>
                                        <span>Monthly question limit: {{ (int)($plan->max_questions_per_month ?? 0) > 0 ? (int)$plan->max_questions_per_month : 'Unlimited' }}</span>
                                    </li>
                                    <li class="feature-item">
                                        <svg class="feature-icon" width="20" height="20" viewBox="0 0 24 24" fill="none"><path d="M5 13l4 4L19 7" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" /></svg>
                                    ...
                                    <li class="feature-item">
                                        <svg class="feature-icon" width="20" height="20" viewBox="0 0 24 24" fill="none"><path d="M5 13l4 4L19 7" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" /></svg>
                                        <span>Images (OCR): {{ (int)($plan->max_images_ocr ?? 0) > 0 ? (int)$plan->max_images_ocr : 'Unlimited' }}</span>
                                    </li>
                                    <li class="feature-item">
                                        <svg class="feature-icon" width="20" height="20" viewBox="0 0 24 24" fill="none"><path d="M5 13l4 4L19 7" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" /></svg>
